...
{"title": "...", "script": "..."}`;
    ...
    try {
      ...
      throw new Error("Failed to parse Gemini script response as JSON: " + cleaned.slice(0, 200));
    }
...
3. "ai_image" — a generated static image (use when a specific, non-stock-able visual is needed but motion isn't important, e.g. a specific product, invented scene, or portrait-like depiction).
4. "ai_video" — generated video with motion (use ONLY when motion is essential to the point being made — this is the most expensive option, use it sparingly, ideally no more than 1 in 5 scenes).
...
- "visual_prompt" is a detailed image/video generation prompt (used only when visual_type is "ai_image" or "ai_video"; can be empty string otherwise).
...
Respond ONLY with valid JSON, no markdown fences, no extra text, in this exact format:
{
  ...
  "scenes": [
    ...
    try {
      ...
      throw new Error("Failed to parse Gemini scene plan response as JSON: " + cleaned.slice(0, 300));
    }
  }
}
